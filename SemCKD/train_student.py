...
def parse_option():
    ...
    # dataset (3 choices are available with us in terms of the dataset)
    ...
    # set the path of model and tensorborda
...
def load_teacher(model_path, n_cls, gpu=None, opt=None):
    ...
    map_location = "cpu" if gpu is None else {'cuda:0': 'cuda:%d' % (gpu if opt.multiprocessing_distributed else 0)}
...
def main():
    ...
    #parsig of the parameters done after that
    opt = parse_option()
    ...
    #it give you the count 
    # number of gpus avialable
    ...
    if opt.multiprocessing_distributed:
        ...
        """
        The concepts of world_size and rank are defined on processes (hence the name process_group). 
        
        If you would like to create 8 processes, then the world_size should be 8, and the ranks for them should range from 0 to 7. 
        
        It is up to the application to determine how to place processes to machines. In the above cluster 
        
        (2 machines, and 4 GPUs each), the best setup would be creating 4 processes on each machine, 
        
        with each exclusively working on a different GPU.
        ...
        # spawn the process with the entry point as the function mentioned in the parameter like here the function name is main_worker
...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    """
    # print("Hola")
    #best accuracy and the total_time
    global best_acc, total_time
    
    # both gpu and gpu_id contains the same value 
    opt.gpu = int(gpu)
    ...
    if opt.multiprocessing_distributed:
        ...
        # for this multiprocessing thing we have to initialize the init_process_group
        dist.init_process_group(backend=dist_backend, init_method=opt.dist_url,
                                world_size=opt.world_size, rank=opt.rank)
        
        #batch size will be divided in all the nodes available
    ...
    if opt.deterministic:
        ...
        # if you set the cudnn.benchmark the CuDNN library will benchmark several algorithms and pick that which 
        # it found to be fastest
        cudnn.benchmark = False
        numpy.random.seed(12345)

    
    #number of classes available in each dataset available in the dictionary
    class_num_map = {
        'cifar100': 100,
        'imagenet': 1000,
        'imagenette': 10,
    ...
    #we are using the scifar 100 dataset
    ...
    ##turn off other layers when doing the inference 
    model_t.eval()
    model_s.eval()
    ...
    #extracting the feature mapfrom the model
    # Taking out the feature 
    feat_t, _ = model_t(data, is_feat=True)
    feat_s, _ = model_s(data, is_feat=True)


    # taking out features from student and the teacher model
    print("\n\n\n")
    print("student first feature shape")
    print(feat_s[1].shape)
    print("\n\n\n")

    print("\n\n\n")
    print("teacher first feature shape")
    print(feat_t[1].shape)
    print("\n\n\n")
    

    # creating the list of models
    ##there are two lists here ModuleList
    ...
    #kd_TTemperature for the KD distillation
    ...
    if opt.distill == 'kd':
        ...
        # leave the first and the last feature map
        ## student model feature 
        # sending out the shape of feature map in s_n
        ...
        """
        #it contains the one part of the shape of the feature map
        print("\n\n\n")
        print("---------------list of shape[1] of features taken out from models----------------")
        print(s_n)
        print("\n\n\n")

        ##Techer model feature        
        ...
        # exit("First break point")
        ## calculates the semckd loss
        criterion_kd = SemCKDLoss()
        
        #calculates the self atttention 
        self_attention = SelfA(len(feat_s)-2, len(feat_t)-2, opt.batch_size, s_n, t_n)
        
        # Both list contains the self attention
    ...
    # criterion loss now contains the losses
    criterion_list.append(criterion_cls)    # classification los
    criterion_list.append(criterion_div)    # KL divergence loss, original knowledge distillation
    criterion_list.append(criterion_kd)     # other knowledge distillation loss
    
    # module list contains the teacher model as well now after attention
    ...
    if torch.cuda.is_available():
        ...
        if opt.multiprocessing_distributed:
            ...
            if opt.gpu is not None:
                ...
                #creating an empty list for the distribute modules
                distributed_modules = []
    ...
    if not opt.skip_validation and False:
